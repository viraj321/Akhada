...
public class RagdollBody {
    ...
    public RagdollBody(float originX, float originY,  int ownerId, float scale) {

        float headLen = 22f * scale, neckLen = 16f * scale, torsoLen = 46f * scale;
        float upperArmLen = 34f * scale, forearmLen = 30f * scale;
        float thighLen = 42f * scale, shinLen = 40f * scale;
        float shoulderWidth = 12f * scale, hipWidth = 10f * scale;
        ...
        leftShoulder  = new PointMass(originX - shoulderWidth, originY - torsoLen);
        leftElbow     = new PointMass(originX - shoulderWidth, originY - torsoLen + upperArmLen);
        leftHand      = new PointMass(originX - shoulderWidth, originY - torsoLen + upperArmLen + forearmLen);

        rightShoulder = new PointMass(originX + shoulderWidth, originY - torsoLen);
        rightElbow    = new PointMass(originX + shoulderWidth, originY - torsoLen + upperArmLen);
        rightHand     = new PointMass(originX + shoulderWidth, originY - torsoLen + upperArmLen + forearmLen);

        leftHip   = new PointMass(originX - hipWidth, originY);
        leftKnee  = new PointMass(originX - hipWidth, originY + thighLen);
        leftFoot  = new PointMass(originX - hipWidth, originY + thighLen + shinLen);

        rightHip  = new PointMass(originX + hipWidth, originY);
        rightKnee = new PointMass(originX + hipWidth, originY + thighLen);
        rightFoot = new PointMass(originX + hipWidth, originY + thighLen + shinLen);
        ...
        constraints.add(new Constraint(chest, leftShoulder, shoulderWidth));
        constraints.add(new Constraint(chest, rightShoulder, shoulderWidth));
        constraints.add(new Constraint(hips, leftHip, hipWidth));
        constraints.add(new Constraint(hips, rightHip, hipWidth));
